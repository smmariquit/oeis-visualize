  ...
  sections: [
    {
      id: "mystery-intro",
      body: [
        "There is a popular image of unsolved mathematics as something locked behind years of graduate school: you need to climb a mountain of definitions before you can even read the question. Some open problems really are like that. The ones in this article are not.",
        "Each of them starts from a rule you can explain to a ten-year-old, run with pencil and paper, and verify by hand in an afternoon. And each of them ends in a question that has defeated every mathematician who has ever tried it.",
        "That combination is not a coincidence, and it is worth taking seriously. A rule being easy to run tells you nothing about whether its long-term behavior is easy to predict. The four rules below, Collatz, Recamán, Kolakoski, and the aliquot map, are among the cleanest demonstrations of that gap anywhere in mathematics.",
        "Every one of them is in this app, so you can generate the data yourself. What nobody can do, so far, is turn the data into a proof.",
      ],
    },
    {
      id: "mystery-collatz",
      ...
      body: [
        "The rule: take any positive whole number. If it is even, halve it. If it is odd, triple it and add one. Repeat. Start at 7 and you get 7, 22, 11, 34, 17, 52, 26, 13, 40, 20, 10, 5, 16, 8, 4, 2, 1.",
        "Start at 27 and the sequence climbs all the way to 9232 before crashing down to 1 after 111 steps. The trajectories rise and fall so erratically that the values are nicknamed hailstone numbers. The Collatz conjecture says that every starting number eventually reaches 1.",
        ...
        "Computers have checked an astonishing range. A distributed computation led by David Bařina verified that every starting value up to $2^{68}$, about 295 quintillion, falls to 1 [2], and the project has since pushed further. It is tempting to read that as overwhelming evidence, and as evidence it is fine. As proof it is worth nothing, and number theory has burned us before.",
        "Pólya's conjecture about prime factors held for every number ever tested until it failed at 906,150,257. The Mertens conjecture was verified to ten trillion and is nevertheless false. A statement about all infinitely many integers simply cannot be settled by checking finitely many of them.",
        "Why does such a small rule resist proof? The honest short answer: the map keeps scrambling the very structure you would use to analyze it. Halving is a statement about a number's binary digits; tripling and adding one is more natural in base 3. Each step shreds whatever pattern the previous step exposed, so the usual tools of number theory, which work by finding structure that persists, get no grip.",
      ...
      figure: {
        ...
        params: { start: 27 },
        caption:
          "The orbit of 27 on a log scale: it spikes to 9232 before collapsing to 1 after 111 steps. Type any start up to a million and tap play to hear your own hailstones fall.",
      },
      anums: ["A006577"],
      links: [
    ...
    },
    {
      ...
      body: [
        ...
        "The open question is coverage: does every nonnegative integer eventually appear? In 1991 Sloane conjectured yes. By 2017 he had publicly changed his mind to \"not so sure,\" which is itself a lesson in how mathematicians hold beliefs [4].",
        "The evidence is unsettling. Benjamin Chaffin, an engineer who took the computation to extraordinary lengths, found that the number 852655 has still not appeared after $10^{612}$ terms [4]. That number of terms is so far beyond physical scale that it needs a trick even to represent: Chaffin's program does not store every visited number individually, which would be impossible, but compresses the visited set into long filled-in runs, which stay manageable because the sequence tends to sweep out solid intervals.",
        "For comparison, the observable universe holds roughly $10^{80}$ atoms. After a search unimaginably past that, 852655 just sits there, unvisited.",
      ...
      figure: {
        ...
        params: {
          ...
          label: "Recamán, first 40 terms",
        },
        caption:
          "Recamán's first forty terms: the walk seesaws between small numbers it doubles back for and ever-higher territory, the same tug-of-war behind the open coverage question.",
      },
      anums: ["A005132"],
      links: [
    ...
    },
    {
      ...
      body: [
        ...
        "The open question sounds like it belongs in a grade-school workbook: in the long run, are half the entries 1s? Nobody knows. It is not even known that the density of 1s converges to anything at all.",
        "The best rigorous information is a bound of Václav Chvátal from 1993, which pins the long-run frequency of 1s, in the limiting sense, between 0.499162 and 0.500838 [5]. Think about what that says: after decades of attention, humanity's knowledge about the fraction of 1s in a sequence anyone can generate on paper is \"somewhere within about a tenth of a percent of one half, probably.\"",
      ...
      figure: {
        kind: "residue-wheel",
        params: { terms: [1, 2, 2, 1, 1, 2, 1, 2, 2, 1, 2, 2, 1, 1, 2, 1, 1, 2, 2, 1] },
        caption:
          "Kolakoski's first twenty terms on the wheel: tap to step and hear it bounce between cells 1 and 2 only, in the run rhythm that describes itself.",
      },
      anums: ["A000002"],
      links: [
    ...
    },
    {
      ...
      body: [
        "The rule: take a number, add up its proper divisors (all divisors except the number itself), and repeat. From 10 you get 1 + 2 + 5 = 8, then 1 + 2 + 4 = 7, then 1, then 0, and the sequence dies.",
        "Perfect numbers like 6 and 28 are fixed points: they equal their own divisor sum. The amicable pair 220 and 284 swap back and forth forever, a two-cycle known since antiquity. Longer loops, called sociable cycles, exist too.",
    ...
    },
    {
      ...
      body: [
        "Look at the shape of the difficulty in each case. Recamán needs the full history of visited numbers to take one more step. Kolakoski's digits and runs define each other, so no level of description is prior to the other. An aliquot step needs the complete factorization of the current term, and factorizations do not compose across steps in any known way.",
