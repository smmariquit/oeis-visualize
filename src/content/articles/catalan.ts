  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        ...
        "Balanced brackets are the skeleton key to the whole collection, because so many other structures are secretly bracket strings. A binary tree can be spelled as brackets: each node becomes a pair enclosing the spelling of its children. A parenthesized product is already made of brackets. Even the handshake problem, in which 2n people around a table shake hands without any two handshakes crossing, is brackets in disguise: walk the circle clockwise and write an opening bracket at each person who starts a handshake with someone ahead, a closing bracket at each person who completes one. Non-crossing is exactly balanced [1].",
      ],
    },
    {
    ...
    },
    {
      ...
      body: [
        ...
        "Here is the promised proof, in prose. Among all walks of n up-steps and n down-steps in some order, sea level rules ignored, there are exactly choose-n-from-2n, since a walk is just a choice of which steps go up. Call a walk bad if it ever dips below sea level. Take any bad walk, find the very first step on which it sinks to one below sea level, and flip every step after that point, ups becoming downs and downs becoming ups. The flipped walk now ends at two below sea level, so it has n − 1 up-steps and n + 1 down-steps. The flip loses no information: given any walk with n − 1 ups and n + 1 downs, it must at some point first reach one below sea level, and flipping its tail there restores the original bad walk. So bad walks correspond exactly to walks with n − 1 ups, of which there are choose n − 1 from 2n. Good walks are what remains: choose-n minus choose-n−1, and that difference simplifies to the central binomial coefficient divided by n + 1. The mysterious formula falls out of a mirror [2].",
        "This reflection trick is a workhorse of combinatorics, and its history carries a small correction worth making. The technique is almost universally called André's reflection method, after Désiré André's 1887 solution of the ballot problem. But André's actual paper contains no reflection at all; he used a different, equally clever cut-and-paste argument, and the reflection proof entered the literature decades later, in 1923, through Jacques Aebly and Dmitry Mirimanoff. The misattribution was documented by Marc Renault in 2008, after generations of textbooks had copied it from one another [2]. Folklore compounds like interest.",
      ],
    },
    {
      id: "catalan-ballot",
