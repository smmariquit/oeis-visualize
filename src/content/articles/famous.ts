  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        ...
        "The open question: are there infinitely many Fibonacci primes? Some Fibonacci numbers are prime, like 2, 3, 5, 13, 89, 233, and 1597, and they keep appearing as far as computers have searched. But despite the sequence's age and fame, nobody can prove the supply never runs out.",
        "The question is sharpened by a lovely known fact: the greatest common divisor of two Fibonacci numbers is itself a Fibonacci number, gcd(F(m), F(n)) = F(gcd(m, n)), which forces any prime Fibonacci number beyond 3 to sit at a prime position. The known Fibonacci primes are collected in A005478, a sequence whose ultimate length is anyone's guess.",
      ],
      figure: {
        kind: "pisano-strip",
        params: { m: 5 },
        caption:
          "A hidden regularity in the portrait: reduce every Fibonacci number mod 5 and the residues cycle forever with Pisano period 20; tap the strip to hear the loop close and start over.",
      },
    ...
    },
    {
      ...
      body: [
        ...
        "The surprising fact: the primes contain arithmetic progressions of every finite length. Take 5, 11, 17, 23, 29: five primes marching in equal steps of 6. Want a hundred primes in equal steps? A million? They exist.",
        "Ben Green and Terence Tao proved this in one of the landmark theorems of the century, published in the Annals of Mathematics in 2008 [2]. The proof is a masterpiece of finding order inside apparent chaos: the primes are irregular enough that no formula tames them, yet regular enough that perfect arithmetic patterns of any length must occur.",
      ...
      figure: {
        ...
        caption:
          "The Ulam spiral: wind 1 to 400 into a square and fill in the primes, and they crowd onto diagonal lines, visible order leaking out of the atoms of multiplication for reasons still not fully understood.",
      },
    ...
    },
    {
      ...
      body: [
        ...
        "The open question: does every nonnegative number eventually get visited? Benjamin Chaffin has computed the sequence to an almost absurd length, 10²³⁰ terms, and one modest number is still missing: 852,655. Every smaller number has been reached; this one never has [3].",
        "Either it appears someday, beyond any computation yet attempted, or it is the smallest of infinitely many numbers the walk skips forever. Sloane has remarked that decades ago everyone assumed every number appears, and that confidence has quietly evaporated. Nobody has a proof in either direction.",
      ],
      figure: {
        kind: "residue-wheel",
        params: { seq: "A005132" },
        caption:
          "The drunkard's walk on a 25-cell wheel: tap to step through Recamán's terms one at a time, watch the backward and forward hops light up the cells, and hear each landing as a pitch.",
      },
      anums: ["A005132"],
    },
    {
      ...
      body: [
        "Take any positive number. If it is even, halve it; if odd, triple it and add one. Repeat. Start from 7 and you get 22, 11, 34, 17, 52, 26, 13, 40, 20, 10, 5, 16, 8, 4, 2, 1.",
        "The Collatz conjecture says every starting number reaches 1 eventually, and A006577 records how many steps each one takes: a wildly jagged sequence with no visible order. The problem is usually credited to Lothar Collatz around 1937, though the written trail is thin and it circulated for decades under other names (Syracuse, Ulam, Kakutani), passed along like folklore. Honest historians call its true origin murky.",
      ...
      figure: {
        ...
        params: { start: 7 },
        caption:
          "The orbit of 7 from the opening paragraph, height on a log scale: sixteen steps of climbing and crashing before it reaches 1; type any start up to a million to draw its orbit, and tap play to hear it.",
      },
      quote: {
    ...
    },
    {
      ...
      body: [
        "The Catalan numbers 1, 1, 2, 5, 14, 42, 132 are combinatorics' favorite answer. How many ways can you write n pairs of brackets so they balance? Catalan. How many mountain ranges can you draw with n up-strokes and n down-strokes? Catalan. Triangulations of polygons, paths that never dip below a diagonal, ways to fully parenthesize a product: all Catalan.",
        "Richard Stanley's book on the subject collects over two hundred distinct things they count [5], which is why they are sometimes called the most ubiquitous numbers after the binomial coefficients.",
        "The surprising fact: they are named after the wrong person, twice over. Eugène Catalan studied them in the 1830s, and Leonhard Euler had already found the formula in 1751, working with Johann Segner and Christian Goldbach. But a century before Catalan, in the 1730s, the Mongolian astronomer and mathematician Minggatu, working in Qing dynasty China on infinite series for trigonometric functions, was already using these numbers routinely.",
        "His manuscript was only published in 1839, and the connection went unnoticed until the historian Luo Jianjin spotted it in 1988 [5]. In mathematics, names tend to honor the best-connected discoverer, not the first.",
        "The open question: primes interact with Catalan numbers in a strangely fragile way. Every odd prime p passes a certain clean divisibility test built from the Catalan number C((p−1)/2). Composite numbers essentially always fail it, with exactly three known exceptions, the so-called Catalan pseudoprimes: 5907, 1194649, and 12327121. The last two are the squares of the only known Wieferich primes, 1093 and 3511, a connection nobody fully understands [6].",
        "Are there infinitely many Catalan pseudoprimes? Is there a fourth one at all? Decades of computer search have found nothing, and no proof exists either way.",
      ],
      figure: {
        kind: "dyck-paths",
        caption:
          "The five mountain ranges you can draw with three up-strokes and three down-strokes, C(3) = 5; tap each thumbnail to see that path spelled out as balanced brackets below.",
      },
      image: {
        ...WIKI_IMAGES.yanghui,
        caption:
    ...
    },
    {
      ...
      body: [
        ...
        "The surprising fact: it is named after the wrong person too. William Kolakoski, an artist with a mathematics degree, posed it as a problem in the American Mathematical Monthly in 1965, and his name stuck. But the sequence had already appeared in 1939, in a paper on symbolic dynamics by the American mathematician Rufus Oldenburger, a full 26 years earlier.",
        "Careful authors now write Oldenburger-Kolakoski, and the OEIS entry records the double parentage. Even in a field with perfect archives, credit is a lottery.",
        "The open question: how many 1s are there? Everyone believes the 1s and 2s each occupy exactly half the sequence in the limit, and the numerical evidence is overwhelming. Yet nobody can prove the density is 1/2; nobody can even prove that the density exists at all.",
        "The best rigorous results are humbling: Václav Chvátal proved in 1993 that the upper density of 1s is below 0.50084, and Johan Nilsson later sharpened this to 0.500080. Fifty years of effort, and the frontier of human knowledge about a sequence a child can generate is the fifth decimal place.",
      ],
      anums: ["A000002"],
    },
    {
